    ...
    internal partial class main : Form
    {
        ...
        public main()
        {
            ...
            for (int i = 0; i < this.Controls.Count; i++)
                Controls[i].KeyUp += this.main_KeyDown;
        ...
        private void main_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.Right:
                    if (!menu.Focused)
                    {
                        tabControl_pages.SelectedIndex = (tabControl_pages.TabCount + tabControl_pages.SelectedIndex + (this.RightToLeftLayout ? -1 : 1)) % tabControl_pages.TabCount;
                    }
                    break;
                case Keys.Left:
                    if (!menu.Focused)
